...
async def durable_sleep_and_get_even_number(call_id: str, params: str) -> str:
    _ = call_id
    try:
        sleep_time = int(params)
    except ValueError:
        raise FatalError("Invalid params: %r" % params)

    return await fragile_sleep_and_get_even_number(sleep_time)


async def shell_func_caller():
    logger.info("shell func caller started")
    try:
        result = await durable_sleep_and_get_even_number("call1", "0")
        logger.info(result)

        try:
            result = await durable_sleep_and_get_even_number("call2", "1")
            logger.info(result)
